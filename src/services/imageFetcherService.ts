...
const VIP_MAP: VipEntry[] = [
  { match: 'stock.adobe.com',   proxy: 'https://downloader.la/adobe-stock-downloader.html',   timeout: 45_000 },
  { match: 'adobe.com',         proxy: 'https://downloader.la/adobe-stock-downloader.html',   timeout: 45_000 },
  { match: 'istockphoto.com',   proxy: 'https://downloader.la/istockphoto-downloader.html',   timeout: 45_000 },
  { match: 'shutterstock.com',  proxy: 'https://downloader.la/shutterstock-downloader.html',  timeout: 45_000 },
  { match: 'gettyimages.com',   proxy: 'https://downloader.la/gettyimages-downloader.html',   timeout: 45_000 },
  { match: 'gettyimages.',      proxy: 'https://downloader.la/gettyimages-downloader.html',   timeout: 45_000 },
  { match: 'alamy.com',         proxy: 'https://downloader.la/alamy-downloader.html',         timeout: 40_000 },
  { match: 'depositphotos.com', proxy: 'https://downloader.la/depositphotos-downloader.html', timeout: 40_000 },
  { match: 'dreamstime.com',    proxy: 'https://downloader.la/dreamstime-downloader.html',    timeout: 40_000 },
  { match: '123rf.com',         proxy: 'https://downloader.la/123rf-downloader.html',         timeout: 40_000 },
  { match: 'vectorstock.com',   proxy: 'https://downloader.la/vectorstock-downloader.html',   timeout: 40_000 },
  { match: 'pond5.com',         proxy: 'https://downloader.la/pond5-downloader.html',         timeout: 40_000 },
];
  ...
  for (const sel of candidates) {
    try {
      const href: string = await page.$eval(
        sel,
        (el: Element): string => (el as HTMLAnchorElement).href || el.getAttribute('href') || '',
      );
...
async function layerVipRouter(targetUrl: string, page: Page): Promise<Buffer | null> {
  ...
  return withRetry(async () => {
    ...
    if (hasCF) { console.warn('[L2] CF — 12s wait'); await new Promise(r => setTimeout(r, 12_000)); }
    ...
    const inputOk: boolean = await page.waitForSelector(inputSel,{timeout:15_000}).then(()=>true).catch(()=>false);
    ...
    await page.waitForSelector('a[download], .download-link, a[href*=".jpg"], a.btn-success',{timeout:35_000}).catch(()=>{});
...
async function layerPicsave(targetUrl: string, page: Page): Promise<Buffer | null> {
  ...
  return withRetry(async () => {
    ...
    const inputSel = 'input[type="url"], input[type="text"], input[name="url"], input[placeholder*="http"]';
...
async function layerNetworkIntercept(targetUrl: string, page: Page): Promise<Buffer | null> {
  ...
  try {
    ...
    page.on('request', (req: HTTPRequest): void => {
      if (['font','stylesheet','media','websocket'].includes(req.resourceType())) req.abort().catch(()=>{});
  ...
  } catch { console.warn('[L4] Timeout — checking buffer...'); }
  finally {
    lock = true;
    await page.setRequestInterception(false).catch(()=>{});
  ...
  if (largestBuf) { console.log(`✅ [L4] ${((largestBuf as Buffer).length/1024).toFixed(1)}KB`); return largestBuf; }
  return null;
}
  ...
  try {
    const candidates: ImageCandidate[] = await page.evaluate((): ImageCandidate[] => {
      ...
      Array.from(document.querySelectorAll('img'))
        .filter(img => (img.naturalWidth||img.width||0)>300 && (img.naturalHeight||img.height||0)>300 && img.src.startsWith('http'))
        .sort((a,b)=>((b.naturalWidth||b.width)*(b.naturalHeight||b.height))-((a.naturalWidth||a.width)*(a.naturalHeight||a.height)))
        .slice(0,5).forEach(img => push('img-tag', img.src));
      return results;
    });
    for (const c of candidates) {
      const buf = await safeFetch(c.url, MIN_SIZE);
      if (buf) { console.log(`✅ [L5] ${(buf.length/1024).toFixed(1)}KB via ${c.type}`); return buf; }
    }
...
async function layerScreenshot(page: Page): Promise<Buffer | null> {
  ...
  try {
    ...
    if (buf.length > MIN_SIZE) { console.log(`✅ [L6] ${(buf.length/1024).toFixed(1)}KB`); return buf; }
...
export async function fetchHighResImage(rawUrl: string): Promise<Buffer> {
  ...
  console.log(`\n🚀 [FETCHER-v9] ▶ ${targetUrl}`);
  if (targetUrl !== rawUrl.trim()) console.log(`🔄 Normalized from: ${rawUrl.trim()}`);
  try {
    state.layer = 'L1';
    ...
    await page.evaluateOnNewDocument((): void => {
      ...
      Object.defineProperty(navigator,'plugins',{get:():unknown[]=>
        [{name:'Chrome PDF Plugin'},{name:'Chrome PDF Viewer'},{name:'Native Client'}],configurable:true});
    ...
    if (!state.success) console.error(`❌ [FETCHER-v9] Failed at: ${state.layer}`);
