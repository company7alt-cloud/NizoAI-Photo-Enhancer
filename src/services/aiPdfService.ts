...
// ─── Premium template CSS registry ───────────────────────────────────────────

type TemplateKey = 'tables' | 'report' | 'formal' | 'creative' | 'minimal' | 'academic' | 'default';
...
function buildHtml(htmlContent: string, tpl: TemplateConfig): string {
...
export async function generateAiPDF(rawMarkdown: string, template: string = 'default'): Promise<string> {
  ...
  const htmlContent = await Promise.resolve(marked.parse(cleanMarkdown));

  // 3. Resolve template
  const tplKey = (template as TemplateKey) in TEMPLATES ? (template as TemplateKey) : 'default';
