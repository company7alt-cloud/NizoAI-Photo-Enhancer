...
exports.forceSubscribeMiddleware = forceSubscribeMiddleware;
...
async function forceSubscribeMiddleware(ctx, next) {
    ...
    if (callbackData && EXEMPT_CALLBACKS.includes(callbackData))
        return next();
    const messageText = ctx.message?.text;
    if (messageText && EXEMPT_COMMANDS.some(cmd => messageText.startsWith(cmd)))
        return next();
    try {
    ...
    const existingUser = await User_1.User.findOne({ telegramId: userId });
...
//# sourceMappingURL=forceSubscribe.js.map