...
exports.handleTypographyCallback = handleTypographyCallback;
...
function buildFormattingKeyboard(fmt) {
    ...
    const rows = [
        [
            // @ts-ignore
            { text: isR, callback_data: 'fmt_align_right', style: 'primary' },
            { text: isC, callback_data: 'fmt_align_center', style: 'primary' },
            // @ts-ignore
            { text: isL, callback_data: 'fmt_align_left', style: 'primary' },
        ],
        [
            // @ts-ignore
            { text: lsTxt, callback_data: 'typo_letter', style: 'primary' },
            { text: lhTxt, callback_data: 'typo_line', style: 'primary' },
        ],
        [
            // @ts-ignore
            { text: b, callback_data: 'style_bold', style: 'primary' },
            { text: it, callback_data: 'style_italic', style: 'primary' },
            // @ts-ignore
            { text: ul, callback_data: 'style_underline', style: 'primary' },
        ],
        [
            // @ts-ignore
            { text: sm, callback_data: 'size_small', style: 'primary' },
            { text: nm, callback_data: 'size_normal', style: 'primary' },
            // @ts-ignore
            { text: lg, callback_data: 'size_large', style: 'primary' },
        ],
        [
            // @ts-ignore
            { text: qt, callback_data: 'style_quote', style: 'primary' },
            { text: dv, callback_data: 'style_divider', style: 'primary' },
            // @ts-ignore
            { text: hl, callback_data: 'style_highlight', style: 'primary' },
        ],
        [
            // @ts-ignore
            { text: clRed, callback_data: 'color_red', style: 'primary' },
            { text: clYel, callback_data: 'color_yellow', style: 'primary' },
            // @ts-ignore
            { text: clBlu, callback_data: 'color_blue', style: 'primary' },
            { text: clDef, callback_data: 'color_default', style: 'primary' },
        ],
        [
            // @ts-ignore
            { text: customLabel, callback_data: 'color_custom', style: 'primary' }
        ],
        [
            // @ts-ignore
            { text: '✅ تطبيق وإضافة للمستند', callback_data: 'fmt_apply', style: 'success' }
        ],
        [
            // @ts-ignore
            { text: '🔙 رجوع', callback_data: 'doc_format_back', style: 'danger' }
        ]
    ];
...
const docMakerCallbackLocks = new Map();
function logDocMakerError(scope, error) {
    console.error(scope, error);
}
function logDocMakerCleanup(scope) {
    return (error) => {
        logDocMakerError(scope, error);
    };
}
async function acknowledgeDocMakerCallback(ctx, data) {
    ...
    ctx.answerCallbackQuery = async (...args) => {
        ...
        return originalAnswerCallbackQuery(...args).catch((error) => {
    ...
    const initialAnswer = data === 'doc_type_image_locked'
        ...
        : undefined;
    await ctx.answerCallbackQuery(initialAnswer).catch((error) => {
        logDocMakerError(`[DocMaker:${data}] initial answerCallbackQuery failed:`, error);
    });
}
function isRapidDocMakerCallback(ctx, data) {
...
function estimateWordCount(lines) {
    let totalChars = 0;
    for (const line of lines) {
        if (!line || line.type === 'image' || line.type === 'image_row' || line.type === 'image_cover')
            continue;
    ...
    return {
        inline_keyboard: [
            [
                // @ts-ignore
                { text: '📤 تصدير الآن', callback_data: 'doc_export_pdf', style: 'primary' },
                { text: '✏️ تعديل سطر', callback_data: 'doc_edit_line', style: 'primary' }
            ],
            [
                // @ts-ignore
                { text: '↩️ إعادة آخر سطر', callback_data: 'doc_undo_last', style: 'primary' },
...
async function renderActiveSessionInner(ctx) {
    const lines = ctx.session.documentLines || [];
    const preview = lines.map((l, i) => {
        if (l.type === 'image_cover')
            return `${i + 1}. 📄 [صورة غلاف]`;
        if (l.type === 'image_row' || l.rowImages)
            return `${i + 1}. 🖼️ [مجموعة صور — ${(l.rowImages || []).length} صور]`;
        if (l.type === 'image')
            return `${i + 1}. 🖼️ [صورة]`;
...
async function handleDocMakerCallbackInner(ctx) {
    ...
    if (data.startsWith('doc_tpl_') && data !== 'doc_tpl_confirm' && data !== 'doc_tpl_back') {
        ...
        const sent = await ctx.replyWithPhoto(new grammy_1.InputFile(png, 'preview.png'), {
            ...
            reply_markup: {
                inline_keyboard: [[
                        // @ts-ignore
                        { text: '✅ موافق', callback_data: 'doc_tpl_confirm', style: 'success' },
                        { text: '🔙 رجوع', callback_data: 'doc_tpl_back', style: 'danger' },
                    ]],
            },
        });
...
💡 كل تعديل يظهر فوراً في شاشة العرض`, { parse_mode: 'HTML' });
    ...
    if (data === 'color_custom') {
        ...
        const promptMsg = await ctx.reply('🎨 <b>أرسل كود اللون بصيغة HEX</b>\nمثال: #E91E63', {
            ...
            reply_markup: {
                inline_keyboard: [[
                        // @ts-ignore
                        { text: '❌ إلغاء', callback_data: 'color_custom_cancel', style: 'danger' }
                    ]]
            }
        });
    ...
    if (data === 'doc_export_pdf') {
        ...
        if (estimatedWords > 4000) {
            await ctx.editMessageText('🚫 <b>المستند كبير جداً!</b>\n\n' +
                ...
                reply_markup: {
                    inline_keyboard: [[
                            { text: '🔙 العودة للجلسة', callback_data: 'doc_back_to_session_keep', style: 'primary' }
                        ]]
                }
            });
            return true;
        ...
        await ctx.editMessageText('📤 <b>تأكيد التصدير</b>\n\n' +
            ...
            reply_markup: {
                inline_keyboard: [[
                        // @ts-ignore
                        { text: `✅ موافق — خصم ${cost} محاولة`, callback_data: 'doc_export_confirm', style: 'success' },
                        { text: '❌ إلغاء', callback_data: 'doc_export_cancel', style: 'danger' }
                    ]]
            }
        });
        return true;
    ...
    if (data === 'doc_export_confirm') {
        ...
        const chargedUser = await User_1.User.findOneAndUpdate({ telegramId, dailyQuota: { $gte: cost } }, { $inc: { dailyQuota: -cost } }, { new: true });
        if (!chargedUser) {
            await ctx.editMessageText('❌ <b>رصيدك غير كافٍ!</b>\n\n' +
                ...
                reply_markup: {
                    inline_keyboard: [[
                            // @ts-ignore
                            { text: '🔙 العودة', callback_data: 'doc_back_to_session_keep', style: 'primary' }
                        ]]
                }
            });
            return true;
        ...
        try {
            ...
            await User_1.User.findOneAndUpdate({ telegramId }, { $inc: { dailyQuota: cost } }, { new: true });
    ...
    if (data === 'doc_export_cancel') {
        ...
        await ctx.editMessageText('↩️ <b>تم إلغاء التصدير.</b>\nيمكنك الاستمرار في تعديل المستند.', {
            ...
            reply_markup: {
                inline_keyboard: [[
                        // @ts-ignore
                        { text: '📤 تصدير PDF', callback_data: 'doc_export_pdf', style: 'primary' }
                    ]]
            }
        });
        return true;
    ...
    if (data === 'doc_end_session') {
        try {
            ...
            await ctx.reply('⚠️ سيتم حذف جميع بيانات مشروعك نهائياً. هل أنت متأكد؟', { reply_markup: { inline_keyboard: [[
                            // @ts-ignore
                            { text: '✅ نعم، إنهاء', callback_data: 'doc_confirm_end', style: 'danger' },
                            { text: '❌ لا، العودة', callback_data: 'doc_cancel_end', style: 'primary' },
                        ]] } });
        }
        catch (e) {
        ...
        await ctx.editMessageText('🎨 <b>تصميم نموذج ملون (خطوة 1/2):</b>\n\nاختر <b>لون خلفية</b> المستند (ألوان هادئة واحترافية):', {
            ...
            reply_markup: {
                inline_keyboard: [
                    [
                        // @ts-ignore
                        { text: 'أسود هادئ 🖤', callback_data: 'doc_bg_#1A1A1A', style: 'primary' },
                        { text: 'رمادي فاتح 🤍', callback_data: 'doc_bg_#F0F2F5', style: 'primary' }
                    ],
                    [
                        // @ts-ignore
                        { text: 'كحلي ليلي 🌌', callback_data: 'doc_bg_#1B263B', style: 'primary' },
                        { text: 'مريمية هادئ 🌿', callback_data: 'doc_bg_#8F9779', style: 'primary' }
                    ],
                    [
                        // @ts-ignore
                        { text: 'بيج كلاسيكي 📜', callback_data: 'doc_bg_#FDF5E6', style: 'primary' },
        ...
        await ctx.editMessageText('🔤 <b>تصميم نموذج ملون (خطوة 2/2):</b>\n\nاختر <b>لون النص</b> المتناسق مع الخلفية:', {
            ...
            reply_markup: {
                inline_keyboard: [
                    [
                        // @ts-ignore
                        { text: 'أبيض ناصع ⚪', callback_data: 'doc_txt_#FFFFFF', style: 'primary' },
                        { text: 'أسود فاحم ⚫', callback_data: 'doc_txt_#000000', style: 'primary' }
                    ],
                    [
                        // @ts-ignore
                        { text: 'رمادي داكن 🔘', callback_data: 'doc_txt_#333333', style: 'primary' },
                        { text: 'ذهبي فاخر ', callback_data: 'doc_txt_#D4AF37', style: 'primary' }
                    ],
                    [
                        // @ts-ignore
                        { text: 'أزرق ملكي 🔵', callback_data: 'doc_txt_#1D3557', style: 'primary' },
        ...
        try {
            ...
            const sent = await ctx.replyWithPhoto(new grammy_1.InputFile(previewBuffer, 'color_preview.png'), {
                ...
                reply_markup: { inline_keyboard: [[
                            // @ts-ignore
                            { text: '✅ موافق', callback_data: 'doc_colored_approve', style: 'success' },
                            { text: '🔙 رجوع', callback_data: 'doc_colored_back', style: 'danger' },
                        ]] },
            });
            ...
            await ctx.editMessageText(`✅ <b>تم حفظ الألوان</b> (خلفية: ${bgColor} · نص: ${txtColor})\n\nاضغط متابعة للمتابعة:`, {
                ...
                reply_markup: { inline_keyboard: [[
                            // @ts-ignore
                            { text: 'متابعة ➡️', callback_data: 'doc_colored_approve', style: 'primary' },
                        ]] },
            }).catch(async (error) => {
    ...
    if (data === 'doc_row_add_image') {
        ...
        await ctx.editMessageText(`✅ تم حفظ الصورة ${rowImages.length}\n\n🖼 أرسل الصورة الإضافية الآن:\n` +
            `تنبيه: يجب ألا يتجاوز حجمها ${rowImages[0].lines} سطر (نفس حجم الأولى)`, { parse_mode: 'HTML', reply_markup: { inline_keyboard: [[
                        // @ts-ignore
                        { text: '🔙 إلغاء وإنهاء السطر', callback_data: 'doc_row_finish', style: 'danger' }
                    ]] } });
        return true;
    }
    ...
    if (data.startsWith('doc_row_caption_') && data !== 'doc_row_caption_skip') {
        ...
        if (rawId === 'temp') {
            ...
            await ctx.editMessageText(`📝 أرسل النص الذي تريده تحت الصورة الحالية:`, { reply_markup: { inline_keyboard: [[
                            // @ts-ignore
                            { text: '❌ تخطي بدون تسمية', callback_data: 'doc_row_caption_skip', style: 'primary' }
                        ]] } });
            return true;
        }
...
async function handleTypographyCallback(ctx, data) {
    try {
...
async function handleTypographyCallbackInner(ctx, data) {
...
async function handleDocMakerMessageInner(ctx) {
    ...
    if (ctx.session.awaitingTypographyValue && ctx.message?.text) {
        ...
        if (ctx.session.tempFormatting && ctx.session.tempLine) {
            const kb = buildFormattingKeyboard(ctx.session.tempFormatting);
            try {
                await ctx.reply(`📝 <b>اختر تنسيق النص:</b>\n\n${ctx.session.tempLine}`, { parse_mode: 'HTML', reply_markup: { inline_keyboard: kb.inline_keyboard } });
            }
            catch (e) {
    ...
    if (foundEmojis.length > 0 && isModernProFont) {
        ...
        // ── STEP 3c: Notify user ────────────────────────────────────────────────
        await ctx.reply(`⚠️ <b>تنبيه: الخط المختار لا يدعم الرموز التعبيرية.</b>
...
💡 <b>هل تريد الرموز؟</b> أنهِ الجلسة الحالية وابدأ مستنداً جديداً باختيار خط يدعم الرموز.`, { parse_mode: 'HTML' });
        await refreshPreview(ctx);
        return true;
    }
...
async function showImageFormatMenuInner(ctx) {
    const rowImages = ctx.session.rowImages || [];
    ...
    const keyboard = [
        ...
        [
            // @ts-ignore
            { text: currentMask === 'circle' ? '✅ دائري' : '⭕ دائري', callback_data: 'doc_img_mask_circle', style: 'primary' },
            { text: currentMask === 'rounded' ? '✅ حواف ناعمة' : '🔲 حواف ناعمة', callback_data: 'doc_img_mask_rounded', style: 'primary' },
            // @ts-ignore
            { text: currentMask === 'square' ? '✅ مربع عادي' : '⬛ مربع عادي', callback_data: 'doc_img_mask_square', style: 'primary' },
        ]
    ];
