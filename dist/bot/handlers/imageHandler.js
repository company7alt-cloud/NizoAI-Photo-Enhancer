...
async function imageHandler(ctx) {
    ...
    try {
        ...
        let user = await User_1.User.findOne({ telegramId: userId });
        if (!user) {
            await ctx.reply('⚠️ يرجى إرسال /start أولاً لتسجيل حسابك.');
            return;
        }
        const isAdmin = userId === ADMIN_ID;
        // 2. Reset quota if 24h have passed
        if (!isAdmin &&
            (!user.lastQuotaReset ||
        ...
        if (!isAdmin && user.dailyQuota <= 0) {
            const resetTime = new Date(user.lastQuotaReset.getTime() + 24 * 60 * 60 * 1000);
            const now = new Date();
        ...
        let fileSize = 0;
        // 4. Detect file type and extract metadata
        if (ctx.msg?.photo) {
        ...
        const quotaDisplay = isAdmin ? '∞ (مدير)' : String(user.dailyQuota);
        const text = `اختر الدقة المطلوبة 🎯\n\n⚡ محاولاتك المتبقية اليوم: ${quotaDisplay} من أصل 5`;
        const keyboard = new grammy_1.InlineKeyboard()
            .row().text('🚀 دقة 2K — محاولة واحدة', 'enhance_2k')
            .row().text('✨ دقة 4K — محاولتان (جودة فائقة)', 'enhance_4k')
            .row().text('🔒 دقة 8K — مقفلة', 'locked_8k');
