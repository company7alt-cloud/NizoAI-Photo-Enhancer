...
async function showFormatSelection(ctx, count, _upscale) {
    ...
    if (isSingle) {
        keyboard.push([
            // @ts-ignore
            { text: '📄 PDF', callback_data: 'fconv_pdf', style: 'primary' },
            { text: '🎨 SVG', callback_data: 'fconv_svg', style: 'primary' },
        ]);
    }
    // @ts-ignore
...
async function callbackHandler(ctx) {
    ...
    if (data === 'claim_daily_reward') {
        try {
            ...
            if (!telegramId)
                return;
            const claimUser = await User_1.User.findOne({ telegramId });
            if (!claimUser)
                return;
            ...
            const updated = await User_1.User.findOneAndUpdate({ telegramId }, {
                $inc: { dailyQuota: 5 },
        ...
        await ctx.reply(`🚀 <b>Pro Enhance — تأكيد</b>\n\n${costMsg}\n\nهل أنت موافق؟`, {
            ...
            reply_markup: {
                inline_keyboard: [
                    [
                        // @ts-ignore
                        { text: '✅ نعم، ابدأ التحسين', callback_data: 'pro_confirm_yes', style: 'success' },
                        { text: '❌ لا، إلغاء', callback_data: 'pro_cancel', style: 'danger' },
                    ],
                ],
            },
        });
    ...
    if (data === 'admin_panel') {
        ...
        await ctx.answerCallbackQuery().catch(() => { });
        // ── [KILL-SWITCH BUTTON] ──
        const { isInternetFetcherEnabled: _ifePanel } = await Promise.resolve().then(() => __importStar(require('../../utils/internetFetcherSettings')));
        const _panelOn = _ifePanel();
    ...
    if (data === 'menu_internet_download') {
        ...
        const { isInternetFetcherEnabled: _ifeA } = await Promise.resolve().then(() => __importStar(require('../../utils/internetFetcherSettings')));
        const _adminA = ctx.from?.id?.toString() === process.env.ADMIN_ID;
        if (!_ifeA() && !_adminA) {
            await ctx.answerCallbackQuery().catch(() => { });
    ...
    if (data === 'magic_enhance_start') {
        ...
        if (!isMagicAdmin && magicUser.dailyQuota < 5) {
            await ctx.reply(`⚠️ رصيدك غير كافٍ!\n` +
                ...
                `رصيدك الحالي: ${magicUser.dailyQuota} محاولة`, { parse_mode: 'HTML' });
            return;
        }
        await User_1.User.findOneAndUpdate({ telegramId: ctx.from.id.toString() }, { $set: { awaitingMagicEnhanceImage: true } });
        await ctx.reply('🪄 <b>تحسين الصورة (AI)</b>\n\n' +
            '📸 أرسل لي الصورة الآن وسأعيد توليدها بجودة استوديو احترافية مع الحفاظ على كل تفاصيلها الأصلية 🚀\n\n' +
            '💸 السعر: 5 محاولات\n' +
            'يمكنك إرسالها كصورة عادية أو كملف للحفاظ على الجودة', {
            parse_mode: 'HTML',
            reply_markup: {
                inline_keyboard: [[{ text: '❌ إلغاء', callback_data: 'cancel_magic_enhance', style: 'danger' }]]
            }
        });
        return;
    }
    if (data === 'cancel_magic_enhance') {
        await ctx.answerCallbackQuery({ text: 'تم الإلغاء ❌' }).catch(() => { });
        await User_1.User.findOneAndUpdate({ telegramId: ctx.from.id.toString() }, { $set: { awaitingMagicEnhanceImage: false } });
        await ctx.deleteMessage().catch(() => { });
        return;
    }
    if (data.startsWith('magic_fmt_')) {
        await ctx.answerCallbackQuery();
        const magicConvUser = await User_1.User.findOne({ telegramId: ctx.from.id.toString() });
        ...
        try {
            ...
            const convertedBuffer = await (0, sharp_1.default)(inputBuffer)
                .toFormat(targetFormat, { quality: 95 })
                .toBuffer();
            await ctx.api.deleteMessage(ctx.chat.id, processingMsg.message_id).catch(() => { });
            const ext = targetFormat === 'jpeg' ? 'jpg' : targetFormat;
            await ctx.replyWithDocument(new grammy_1.InputFile(convertedBuffer, `NizoAI_${ext.toUpperCase()}_${Date.now()}.${ext}`), { caption: `✅ تم التحويل إلى ${ext.toUpperCase()} بنجاح` });
        }
        catch (err) {
            await ctx.api.deleteMessage(ctx.chat.id, processingMsg.message_id).catch(() => { });
            await ctx.reply('❌ فشل التحويل. حاول مرة أخرى.');
    ...
    if (data === 'action_referral_sub') {
        await ctx.answerCallbackQuery().catch(() => { });
        const invites = await User_1.User.countDocuments({
        ...
        await ctx.editMessageCaption({
            caption: `🔗 <b>رابط الدعوة الخاص بك</b>\n\n` +
