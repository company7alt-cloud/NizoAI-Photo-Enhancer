...
imageBot.command('reset', async (ctx) => {
    await ctx.reply('⚠️ تأكيد إعادة التشغيل\n\n' +
        ...
        reply_markup: {
            inline_keyboard: [
                // @ts-ignore
                [{ text: '✅ نعم، أعد التشغيل', callback_data: 'action_confirm_reset', style: 'success' }],
                [{ text: '❌ تراجع', callback_data: 'action_cancel_reset', style: 'danger' }],
            ],
        },
    });
});
...
imageBot.on('message:text', async (ctx, next) => {
        ...
        if (inputType === 'search_user') {
            ...
            await ctx.reply(`🔍 <b>معلومات المستخدم</b>\n\n🆔 ID: <code>${foundUser.telegramId}</code>\n👤 Username: @${foundUser.username || 'غير محدد'}\n⚡ المحاولات: ${foundUser.dailyQuota}\n🚫 محظور: ${foundUser.isBanned ? 'نعم' : 'لا'}`, {
                ...
                reply_markup: {
                    inline_keyboard: [
                        // @ts-ignore
                        [{ text: '🚫 حظر', callback_data: `admin_ban_${foundUser.telegramId}`, style: 'primary' }],
                        [{ text: '🔓 رفع الحظر', callback_data: `admin_unban_${foundUser.telegramId}`, style: 'primary' }],
                        // @ts-ignore
                        [{ text: '➕ إضافة محاولات', callback_data: `admin_addattempts_${foundUser.telegramId}`, style: 'primary' }],
                    ],
                },
            });
            return;
            ...
            try {
                ...
                const msg = await ctx.api.sendMessage(channelId, giveawayText, {
                    ...
                    reply_markup: {
                        inline_keyboard: [[
                                // @ts-ignore
                                { text: '🍀 جرب حظك الآن 🟢', callback_data: 'gw_roll_init', style: 'primary' }
                            ]]
                    }
                });
                ...
                await ctx.reply(`✅ <b>تم نشر التوزيعة بنجاح!</b> 🎉\n\n` +
                    ...
                    reply_markup: {
                        inline_keyboard: [[
                                { text: '📤 عرض رسالة التوزيعة', url: `https://t.me/${safeChannel}/${msg.message_id}`, style: 'primary' }
                            ]]
                    }
                });
            }
    ...
    if (isAdm && isFundCampaignPending(ctx.from.id)) {
        ...
        if (result.status === 'ask_target') {
            // @ts-ignore
            await ctx.reply(`✅ تم التحقق من صلاحيات البوت.\n\nكم عدد الأعضاء المطلوب؟`, { reply_markup: { inline_keyboard: [[{ text: '↩️ رجوع', callback_data: 'cancel_fund_campaign', style: 'danger' }]] } });
        }
        ...
        if (activeUser) {
            ...
            await ctx.reply(`📤 <b>هل أنت متأكد من إرسال هذا الرد للعميل؟</b>\n\n` +
                ...
                reply_markup: {
                    inline_keyboard: [[
                            // @ts-ignore
                            { text: '✅ نعم، أرسل للعميل', callback_data: `confirm_support_send_${activeUser.telegramId}`, style: 'success' },
                            { text: '❌ لا، إلغاء الإرسال', callback_data: 'cancel_support_send', style: 'danger' }
                        ]]
                }
            });
    ...
    if (ctx.session?.awaitingInternetLink) {
        ...
        if (!_ifeB() && !_adminB) {
            await ctx.reply(`🔧 *تحميل الصور من الإنترنت*\n\n` +
                `✨ هذه الميزة تحت الصيانة حالياً لتقديم تجربة أفضل لك!\n\n` +
                `🚀 سيتم إعادة تفعيلها قريباً إن شاء الله 🌟\n` +
                `💙 نعتذر عن الإزعاج ونقدّر صبرك الجميل`, { parse_mode: 'Markdown' });
            return;
        }
        // ── Smart CDN Resolver ──
        const { resolveCdnToPageUrl } = await Promise.resolve().then(() => __importStar(require('./services/imageFetcherService')));
        const resolvedLink = resolveCdnToPageUrl(link);
        if (resolvedLink !== link) {
            const cdnMsg = await ctx.reply('🔄 <b>تم اكتشاف رابط CDN مباشر!</b>\n\n' +
                '🧠 جاري التحويل التلقائي إلى رابط الصفحة الأصلية...', { parse_mode: 'HTML' });
            link = resolvedLink;
            await new Promise(r => setTimeout(r, 1_500));
            await ctx.api.deleteMessage(cdnMsg.chat.id, cdnMsg.message_id).catch(() => { });
        }
        else if (/istockphoto\.com\/.*\.(jpg|jpeg|png)/i.test(link) ||
        ...
        const processingMsg = await ctx.reply('🔍 <b>جاري فحص الرابط والبحث عن الصورة...</b>', { parse_mode: 'HTML' });
        const waitMessages = [
        ...
        const fetchInterval = setInterval(() => {
            ...
            ctx.api.editMessageText(processingMsg.chat.id, processingMsg.message_id, waitMessages[msgIndex], { parse_mode: 'HTML' }).catch(() => { });
        }, 3500);
        try {
            ...
            const formatKeyboard = {
                inline_keyboard: [
                    [
                        { text: '🖼️ JPG', callback_data: 'magic_fmt_jpg', style: 'primary' },
                        { text: '🖼️ PNG', callback_data: 'magic_fmt_png', style: 'primary' },
                        { text: '🖼️ WEBP', callback_data: 'magic_fmt_webp', style: 'primary' },
                    ],
                    [
                        { text: '🖼️ AVIF', callback_data: 'magic_fmt_avif', style: 'primary' },
                        { text: '🖼️ TIFF', callback_data: 'magic_fmt_tiff', style: 'primary' },
                    ],
                ],
            };
            ...
            await ctx.reply('📤 <b>هل تريد مشاركة هذا البلاغ مع مطور البوت؟</b>\n\n' +
                ...
                reply_markup: {
                    inline_keyboard: [
                        [
                            // @ts-ignore
                            { text: '✅ نعم، أرسل البلاغ', callback_data: `confirm_report_${chatId}_${messageId}`, style: 'success' },
                            { text: '❌ لا، إلغاء', callback_data: 'cancel_report_confirm', style: 'danger' },
                        ],
                    ],
                },
            });
        ...
        if (activeUser) {
            await ctx.reply(`📤 <b>هل تريد إرسال هذا الملف/الصورة للعميل؟</b>\n\n` +
                ...
                reply_markup: {
                    inline_keyboard: [[
                            // @ts-ignore
                            { text: '✅ نعم، أرسل الملف', callback_data: `confirm_support_send_${activeUser.telegramId}`, style: 'success' },
                            { text: '❌ لا، إلغاء', callback_data: 'cancel_support_send', style: 'danger' }
                        ]]
                }
            });
...
registerDocCallback('doc_admin_toggle_welcome', 'doc_admin_toggle_welcome', async (ctx) => {
    ...
    await ctx.editMessageText(`🔧 <b>لوحة تحكم المشرف</b>\n\nحالة البوت: ${docBotLocked ? '🔒 مقفول' : '🔓 مفتوح'}`, { parse_mode: 'HTML', reply_markup: getDocAdminKeyboard() }).catch((error) => logDocBotError('[DocBot:doc_admin_toggle_welcome] editMessageText failed:', error));
});
registerDocCallback('doc_admin_lock', 'doc_admin_lock', async (ctx) => {
    ...
    await ctx.editMessageText(`🔧 <b>لوحة تحكم المشرف</b>\n\nحالة البوت: ${docBotLocked ? '🔒 مقفول' : '🔓 مفتوح'}`, { parse_mode: 'HTML', reply_markup: getDocAdminKeyboard() }).catch((error) => logDocBotError('[DocBot:doc_admin_lock] editMessageText failed:', error));
});
...
registerDocCallback('start_free_ai', 'start_free_ai', async (ctx) => {
    ...
    await ctx.answerCallbackQuery().catch(() => { });
    await ctx.reply('🆓 Ai Free PDF — اختر النوع:\n\n' +
...
registerDocCallback('start_template_pdf', 'start_template_pdf', async (ctx) => {
    ...
    await ctx.reply(`👑 <b>PRO — اختر قالب التصميم:</b>`, {
        parse_mode: 'HTML',
        reply_markup: {
            inline_keyboard: [
                [
    ...
    if (ctx.session.templateWorkflowState === 'collecting_text') {
        ...
        if (!ctx.message?.text) {
            await ctx.reply('⚠️ عذراً، لا يمكن استقبال الصور أو الملصقات أو الوسائط في هذه المرحلة. أرسل نصوصاً فقط.', {
                reply_markup: { inline_keyboard: [[{ text: 'إلغاء ❌', callback_data: 'tpl_cancel', style: 'danger' }]] }
            });
            return;
        }
        const text = ctx.message.text.trim();
...
registerDocCallback('start_premium_ai', 'start_premium_ai', async (ctx) => {
    ...
    // TASK 3: Show mode selection before existing flow
    await ctx.answerCallbackQuery().catch(() => { });
    await ctx.reply('🤖 NizoAI PDF — اختر النوع:\n\n' +
        '1️⃣ تلقائي — البوت يولّد المستند فوراً بدون صور مخصصة\n' +
        '2️⃣ احترافي ✨ — أنت ترفع صورك وتتحكم بكل صفحة\n' +
        '(النسخة الاحترافية متاحة مرة واحدة فقط مجاناً)', {
        reply_markup: {
...
registerDocCallback('nizo_pdf_auto', 'nizo_pdf_auto', async (ctx) => {
    ...
    await ctx.reply(`🤖 <b>NizoAI PDF</b> — اختر طراز المستند:`, {
        parse_mode: 'HTML',
        reply_markup: {
            inline_keyboard: [
                [
...
NIZOPDF_STYLES.forEach(style => {
    registerDocCallback(`nizopdf_style_${style}`, `nizopdf_style_${style}`, async (ctx) => {
        ...
        ctx.session.totalWords = 0;
        ctx.session.estimatedPages = 0;
        await ctx.editMessageText(`✅ <b>الطراز: ${style.toUpperCase()}</b>\n\n` +
            `📝 أرسل المحتوى رسالة رسالة. عند الانتهاء أرسل \u202a<b>تم</b>\u202c أو اضغط إلغاء.`, { parse_mode: 'HTML' }).catch(() => { });
    });
});
...
function buildProPageKeyboard(completedPages = []) {
    ...
    return {
        inline_keyboard: [
            ...
            [
                // @ts-ignore
                { text: '✅ موافق', callback_data: 'pro_confirm', style: 'success' },
                // @ts-ignore
                { text: '❌ إلغاء', callback_data: 'cancel', style: 'danger' }
            ]
        ]
...
registerDocCallback('free_pdf_pro', 'free_pdf_pro', async (ctx) => {
    ...
    if (user?.usedProMode) {
        await ctx.answerCallbackQuery().catch(() => { });
        await ctx.reply('⚠️ لقد استخدمت النسخة الاحترافية مرة واحدة بالفعل.\n' +
...
for (let pageN = 1; pageN <= 5; pageN++) {
    ...
    registerDocCallback(`pro_page_done_${N}`, `pro_page_done_${N}`, async (ctx) => {
        ...
        if (!ctx.session.proImageData)
            ctx.session.proImageData = [];
        const existingPage = ctx.session.proImageData.find((p) => p.page === N);
        if (!existingPage) {
        ...
        if (ctx.session.proImageMessageId && ctx.chat?.id) {
            await ctx.api.editMessageReplyMarkup(ctx.chat.id, ctx.session.proImageMessageId, { reply_markup: buildProPageKeyboard(completedPages) }).catch(() => { });
        }
...
registerDocCallback('pro_confirm', 'pro_confirm', async (ctx) => {
    ...
    const pagesWithPhotos = proData.filter((p) => p.photos && p.photos.length > 0);
    ...
    try {
        const { generateProImagePDF } = await Promise.resolve().then(() => __importStar(require('./services/aiPdfService')));
        const pdfPath = await generateProImagePDF({
            topic,
            images: proData,
            botToken: process.env.DOC_BOT_TOKEN || process.env.BOT_TOKEN || '',
        });
        await ctx.replyWithDocument(new grammy_1.InputFile(pdfPath, `ProDoc_${Date.now()}.pdf`), { caption: '✅ مستندك الاحترافي جاهز! 📄✨' });
        // Mark pro mode as used
        await User_1.User.findOneAndUpdate({ telegramId: userId }, { usedProMode: true });
...
docBot.on(['message:photo', 'message:document'], withDocBotHandler('pro_image_collector', async (ctx, next) => {
    ...
    if (!ctx.session.proImageData)
        ctx.session.proImageData = [];
    let pageData = ctx.session.proImageData.find((p) => p.page === pageN);
    if (!pageData) {
...
registerDocCallback('nizopdf_done', 'nizopdf_done', async (ctx) => {
    if (ctx.session.awaitingMoreText) {
        ...
        await ctx.editMessageText(`✅ <b>تم تلقي المحتوى</b>\n` +
            ...
            reply_markup: {
                inline_keyboard: [
                    [
                        // @ts-ignore
                        { text: '🔒 1 صفحة', callback_data: 'pages_locked', style: 'primary' },
                        // @ts-ignore
                        { text: '🔒 2 صفحة', callback_data: 'pages_locked', style: 'primary' },
                        // @ts-ignore
                        { text: '🔒 3 صفحات', callback_data: 'pages_locked', style: 'primary' },
                        // @ts-ignore
                        { text: '🔒 5 صفحات', callback_data: 'pages_locked', style: 'primary' },
                    ],
                    [
                        // @ts-ignore
                        { text: '🔒 10 صفحات', callback_data: 'pages_locked', style: 'primary' },
                        // @ts-ignore
                        { text: '🔒 15 صفحة', callback_data: 'pages_locked', style: 'primary' },
                        // @ts-ignore
                        { text: '🔒 20 صفحة', callback_data: 'pages_locked', style: 'primary' },
                    ],
                    [
                        // @ts-ignore
...
docBot.on('message:text', withDocBotHandler('text_input', async (ctx, next) => {
    ...
    if (ctx.session.awaitingMoreText && ctx.message?.text) {
        ...
        if (incoming === 'تم' || incoming === 'تم.' || incoming === 'انتهيت') {
            ...
            const estimatedPages = Math.ceil(totalWords / 250);
            ctx.session.totalWords = totalWords;
            await ctx.reply(`✅ <b>تم تلقي المحتوى</b>\n` +
                `📝 إجمالي الكلمات: ${totalWords} — الصفحات المتوقعة: ~${estimatedPages}\n\n` +
                `<b>اختر عدد الصفحات:</b>`, {
                parse_mode: 'HTML',
                reply_markup: {
        ...
        if (isPhoto || isImageDoc) {
            ...
            if (ctx.session.tempImage?.fileId) {
                await ctx.reply('⚠️ <b>أكمل إعدادات الصورة الحالية أولاً</b>\nأو اضغط إلغاء الصورة.', {
                    ...
                    reply_markup: {
                        inline_keyboard: [
                            // @ts-ignore
                            [{ text: '🔙 إلغاء الصورة والعودة', callback_data: 'doc_back_to_session', style: 'danger' }]
                        ]
                    }
                });
                return;
            ...
            await ctx.reply('🖼 <b>تم استلام الصورة!</b>\n\n📏 كم سطراً تريد تخصيصها للصورة في المستند؟\nأو اجعلها غلافاً يملأ الصفحة بالكامل:', {
                ...
                reply_markup: {
                    inline_keyboard: [
                        // @ts-ignore
                        [{ text: '📄 ملء الصفحة كاملة (غلاف)', callback_data: 'doc_img_full_cover', style: 'primary' }],
                        [{ text: '📏 افتراضي — 5 أسطر', callback_data: 'doc_img_space_5', style: 'primary' }],
                        // @ts-ignore
                        [{ text: '📐 كبير — 10 أسطر', callback_data: 'doc_img_space_10', style: 'primary' }],
                        [{ text: '✍️ تخصيص العدد...', callback_data: 'doc_img_space_custom', style: 'primary' }],
                        // @ts-ignore
                        [{ text: '🔙 إلغاء', callback_data: 'doc_back_to_session', style: 'danger' }]
                    ]
                }
            });
            return;
...
function parseImageSections(markdown) {
