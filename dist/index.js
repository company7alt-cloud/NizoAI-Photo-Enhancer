...
docBot.callbackQuery('premium_auto_analyze', async (ctx) => {
    ...
    ctx.session.awaitingPremiumImage = true;
    await ctx.reply('⚡ <b>وضع التحليل التلقائي</b>\n\n' +
        '🖼 أرسل صورة النموذج أو المستند المطلوب\n' +
        'سيقوم NizoAI بتحليله تلقائياً وتحديد عدد الصفحات وخصم النقاط', { parse_mode: 'HTML' });
});
...
- جودة عالية تصلح للطباعة والتقديم الرسمي`;
        ...
        await ctx.replyWithDocument(new grammy_1.InputFile(pdfBuffer, fileName), { caption: captionText, parse_mode: 'HTML' });
    }
    catch (err) {
        await User_1.User.updateOne({ telegramId }, { $inc: { dailyQuota: cost } });
...
docBot.on(['message:photo', 'message:document'], async (ctx, next) => {
    if (ctx.session.awaitingPremiumImage) {
        ...
        try {
            ...
            if (userPrompt)
                ctx.session.pendingPremiumPrompt = userPrompt;
            if (ctx.session.premiumAutoMode) {
                ...
                ctx.session.awaitingPremiumText = true;
                await ctx.api.deleteMessage(ctx.chat.id, waitMsg.message_id).catch(() => { });
                await ctx.reply('✅ <b>تم استلام صورة النموذج بنجاح.</b>\n\n' +
